...
import pickle
import sys
sys.path.append("D:\\MachineLearning\\0-Code_Test\\1-baselines")
...
"""Research the result on tensorboard"""
...
def train(env, nb_epochs, nb_epoch_cycles, render_eval, reward_scale, render, param_noise, actor, critic,
    ...
    with U.single_threaded_session() as sess:
        ...
        for epoch in range(nb_epochs):
            for cycle in range(nb_epoch_cycles): # the same init position cycle 20 times
                # Perform rollouts.
                for t_rollout in range(nb_rollout_steps):
                    ...
                    # Execute next action.
                    if rank == 0:
                        env.plot_model(render)
                    assert max_action.shape[0] == action.shape[0]
